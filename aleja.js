...
drink.addEventListener('click', async (event) => {
  ...
  try {
    ...
    // Mostrar el cóctel en tu HTML
    if (cocktail && cocktail.length > 0) {
      const drinkData = cocktail[0]; // La API devuelve un array
      
      // Mostrar mensaje primero
      bell.innerHTML = '<p><strong>Here is your drink. Enjoy it!! </strong></p>';
      
      // Después de 1.5 segundos, mostrar la receta completa
      setTimeout(() => {
        bell.innerHTML = `
...
// Función para obtener el cóctel
